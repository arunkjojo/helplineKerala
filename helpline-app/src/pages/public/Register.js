    ...
    registerForm = (e)=>{
        ...
        if(dat.user && dat.mobile && dat.password && dat.usrtype && dat.dist && dat.localbody && dat.area && dat.blood){
            ...
            if(dat.user.length > 0 && dat.usrtype.length > 0 && dat.dist.length > 0 && dat.localbody.length > 0 && dat.area.length > 1 && dat.blood.length > 0 && (dat.password.length >=8  && dat.password.length <= 16) && dat.mobile.length === 10){
                if (!navigator.onLine) {
                    toastr.warning("No Internet Connection, Register is not support, Please Check Your Internet Connection", "Application is Offline Mode");
                }else{
            ...
            <div className="container">
                <form className="form-group" noValidate>
                    <div className="form-group">
                        ...
                        <Input type="text" autoFocus value={this.state.userName} onChange={this.inputSet}  className="form-control" id="username" name="userName" placeholder="Enter your Name" required/>
                    </div>
                    <div className="form-group">
                        <label htmlFor="mobile">Mobile Number</label>
                        <Input type="text" pattern="[0-9]*" onKeyPress={(event) => {if (!/[0-9]/.test(event.key)) {toastr.warning("Only numbers are allowed", "Invalid Mobile");event.preventDefault();} }} onChange={this.inputSet} className="form-control" id="mobile" name="mobileNumber" placeholder="Enter your Mobile number" required noValidate/>
                    </div>
                    <div className="form-group">
                    ...
                    <div className="form-group">
                        <Button variant="contained" color="secondary" onClick={this.registerForm} className="form-control btn btn-success">Register</Button>
                    </div>
                </form>
                <div className="container">
